...
opt_parser <- OptionParser(
  option_list = list(
    ...
    make_option(
        c("-x", "--extra_metadata"),
        type = "character",
        help = "Option to add a csv file containing metadata from sequencing but not on Genbank"
    ),
    make_option(
        c("-z", "--extra_fasta"),
        type = "character",
        help = "Option to add a fasta file containing metadata from sequencing but not on Genbank"
    ),
    make_option(
        c("-o", "--output_dir"),
        type = "character",
...
metadata.df <- metadata.df %>%
  mutate(serotype = case_when(
    grepl("dengue virus 2|Dengue virus 2|dengue virus type 2", `Virus Name`, ignore.case = TRUE) ~ "Dengue_2",
...
for (serotype in c("Dengue_1", "Dengue_2", "Dengue_3", "Dengue_4")) {
  ...
  write.fasta(
    ...
    file.out = paste0(opt$output_dir, "/Unaligned_", serotype, ".fasta")
  )
  ...
  write.table(taxa_split,
    file = paste0(opt$output_dir, "/", serotype, "_infoTbl.txt"),
    sep = "\t",
    col.names = TRUE,
    row.names = FALSE,
    quote = FALSE
  ...
  write.csv(taxa_split,
    file = paste0(opt$output_dir, "/", serotype, "_infoTbl.csv"),
    row.names = FALSE
  )
