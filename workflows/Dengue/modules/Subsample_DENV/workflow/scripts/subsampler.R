...
opt_parser <- OptionParser(
  option_list = list(
    ...
    make_option(
        c("-c", "--country"),
        type="character",
        help="Country(ies) of Interest, the first country specified will determine local region"
    ),
    make_option(
        c("-n", "--number_sequences"),
        type="numeric",
        help="Number of desired sequences (defult is 1:1 ratio with number of sequences from desired country(ies))"
    ),
    make_option(
        c("-e", "--prop_rd"),
        type="numeric",
        default = 0.8,
        help="Set proportion desired for region of interest (default is 0.8)"
    ),
    make_option(
        c("-w", "--prop_or"),
        type="numeric",
        default = 0.2,
        help="Set proprotion desired for outside region of interest (default is 0.2)"
    ),
    make_option(
        c("-o", "--output_dir"),
        type="character",
        help="Output directory for subsampled sequences and metadata"
    )
  )
)
...
ggsave(paste0(opt$output_dir, '/subsampled_', serotype, '_summary_plot.pdf'), width = 10, height = 10, units = "in")
...
write.fasta(sequences=seqs[vec.tokeep], names=names(seqs)[vec.tokeep],
            file.out=paste0(opt$output_dir, '/subsampled_', serotype, '.fasta'))

write.csv(taxa_split,
          file = paste0(opt$output_dir, "/subsampled_",serotype,'_infoTbl.tsv'),
          row.names = FALSE, quote=FALSE)

write.csv(taxa_split,
          file = paste0(opt$output_dir, "/subsampled_",serotype,'_infoTbl.csv'),
          row.names = FALSE,quote=FALSE)
